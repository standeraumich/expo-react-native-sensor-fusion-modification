...
const types = [Sensors.Gyroscope, Sensors.Accelerometer, Sensors.Magnetometer];

const SensorFusionContext = React.createContext(null);

const createFilters = () =>
    types.map(() => [...Array(3)].map(() => new KalmanFilter()));
const SensorFusionProvider = ({ children, ...extraProps }) => {
    const { sampleInterval } = extraProps;
    const [ahrs, setAhrs] = useState(new Ahrs(extraProps));
    ...
    useEffect(
        () =>
            types
                .map((type) => type.setUpdateInterval(50))
                .reduce(() => undefined)
        // [ sampleInterval ],
    );
    useEffect(() => {
        const subscriptions = types.map((type, i) =>
            type.addListener(({ x, y, z }) => {
                [x, y, z].map((e, j) => (get[i][j] = filters[i][j].filter(e)));
                ahrs.update(...get[0], ...get[1], ...get[2]);
                if (i === types.length - 1) {
                    setValue({
                        ahrs,
                        gyro,
